...
function getCodeTime() { // returns a single integer time for use in code
  time = new Date();
  return time.getTime();
}
function mobile() { // Checks whether user is on mobile copied from http://detectmobilebrowsers.com
  var check = false;
...
var protectedStorage = function() { // storage that limits modification
  var text = "";
...
// Prototypes
String.prototype.count = function(char) { // counts how many times a character is found in a string
  count = 0;
...
String.prototype.download = function(name) { // downloads a string
  a = document.createElement("a");
  a.href = "data:text/plain,"+encodeURIComponent(this.valueOf());
  a.download = name;
  a.click();
}
String.prototype.backspace = function() { // returns the string with the last character removed
  temp = this.valueOf();
  text = "";
